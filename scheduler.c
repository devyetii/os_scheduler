    ...
    while (1)
    {
        ...
        if (__down(sem_id) != -1)
        {
            ...
            while (recievedProcess.pid != -1)
            {
                ...
                // create the PCB
                PCB *p = PCB__create(recievedProcess, recievedProcess.t_running, 0, 0, IDLE, 0, 0);
        ...
        if (cur_clock > old_clock && process_in_progress == NULL && !FIFOQueue__isEmpty(fq))
        {
            ...
            if (p->p_data.t_arrival <= cur_clock)
            { // (<) just for saftey
        ...
        // When to exit
        if (finished && process_in_progress == NULL && FIFOQueue__isEmpty(fq))
            break;
        // Save latest clock
        old_clock = cur_clock;
    }
